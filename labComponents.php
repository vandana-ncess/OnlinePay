          ...
          <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" onsubmit="validate()">
             ...
             <fieldset class="field" style="width:820px;" ><legend align="left" style="padding-left:10px;padding-left:10px;"><h4>Laboratory Services</h4></legend>
          	<table  height="80px" width="800px">
                ...
                <tr>
                    <td height="20px;"> <input type="text" value="<?php echo isset($_SESSION['labService']) ? $_SESSION['labService'] : ''; ?> " id="txtService" 	name="txtService" style="width:250px;height:22px;" readonly="readonly" required />
             ...
             <?php
				$sql = "SELECT A.labServiceID, labServiceName, charge, addnCharge, labServiceStatus FROM lab_master A JOIN lab_rates B on A.labServiceID = B.labServiceID WHERE template='simple'";
...
<?php 
		...
		if(isset($_POST["btnAdd"]))
		{
			...
			if(mysqli_num_rows($result1) > 0)
			{
				...
				$sql = "INSERT INTO lab_master(labServiceName,labServiceStatus,template) VALUES('" . $_POST['txtService'] . "','Active','". $_SESSION['template'] ."')";
				$result = mysqli_query($conn,$sql); 
				$last_id = mysqli_insert_id($conn);
				$sql = "INSERT INTO lab_rates(labServiceID,charge,addnCharge,category,status,mandatory) VALUES(". $last_id . "," . $_POST['txtCharge'] . "," . $_POST['txtAddnCharge'] . ",'sample','Active','Mandatory')";
				echo $sql;
				$result = mysqli_query($conn,$sql); 
				if($result)
					...
					echo "Unable to save!";
				echo "<script> window.location='labComponents.php';</script>";
					$_POST['btnAdd'] = '';
			}
			else
			{
			
				$sql = "UPDATE lab_rates SET charge = " . $_POST['txtCharge'].trim() . " , addnCharge = " . $_POST['txtAddnCharge'].trim()  . " WHERE labServiceID =  " . $_POST['txtServiceID'] ;
echo $sql;
				$result = mysqli_query($conn,$sql); 
				if($result)
