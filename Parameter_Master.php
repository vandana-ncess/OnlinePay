...
?>
       ...
       <div class="lab" align="center"  style="width:970px; height:80px;">
          <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
          	<table  height="80px" width="700px">
                ...
                <tr><td height="20px;">Name of Parameter</td>	
                    <td height="20px;"> <input type="text" id="txtParameter" 	name="txtParameter" style="width:250px;height:22px;" required />
                    <input type="hidden" id="txtParameterID" name="txtParameterID" /> <input type="hidden" id="txtMode" name="txtMode" value="Add" /></td>
          ...
          <?php
				...
				{
			?>
          	<table title="Parameters" id="tblParameter">
                    	<thead style="background:#3A8E00; color:#FFF;">
                        	<tr>
                            	<th height="30px" colspan="7" ><strong>Parameters</strong></th>
                            </tr>
                        </thead>
  ...
  <?php 
		...
		{
			$sql = "SELECT * FROM parameter_master WHERE parameterName = '" . $_POST['txtParameter'] . "' ";
			echo $sql;
			...
			if(mysqli_num_rows($result1) > 0)
			{
				...
				$sql = "INSERT INTO parameter_master(parameterName,parameterStatus) VALUES('" . $_POST['txtParameter'] .  "',1)";
				$result = mysqli_query($conn,$sql); 
				echo $sql;
				if($result)
					...
					echo "Unable to save!";
				echo "<script> window.location='Parameter_Master.php';</script>";
					$_POST['btnAdd'] = '';
  ...
  function delService(x) {
	...
	++index;
	var serviceID = document.getElementById("tblParameter").rows[index].cells.item(4).innerHTML;
	...
	if(confirm('Are you sure you want to delete this Service?'))
	{
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
				document.location='Parameter_Master.php';
			}
        };
        xmlhttp.open("GET","del_service.php?service=param&q="+serviceID,true);
        xmlhttp.send();
	}
