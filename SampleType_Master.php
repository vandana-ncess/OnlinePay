...
?>
       ...
       <div class="lab" align="center"  style="width:970px; height:80px;">
          <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
          	<table  height="80px" width="700px">
                ...
                <tr><td height="20px;">Sample Type</td>	
                    <td height="20px;"> <input type="text" id="txtSamp" 	name="txtSamp" style="width:250px;height:22px;" required />
                    <input type="hidden" id="txtSampID" name="txtSampID" /> <input type="hidden" id="txtMode" name="txtMode" value="Add" /></td>
          ...
          <?php
				$sql = "SELECT * FROM sampletype_master";
	            $result = mysqli_query($conn,$sql); 
		   		if(mysqli_num_rows($result) > 0)
				{
			?>
          	<table title="Sample Type" id="tblSamp">
                    	<thead style="background:#3A8E00; color:#FFF;">
                        	<tr>
                            	<th height="30px" colspan="7" ><strong>Sample Type</strong></th>
                            </tr>
                        </thead>
  ...
  <?php 
		...
		{
			$sql = "SELECT * FROM sampletype_master WHERE sampleType = '" . $_POST['txtSamp'] . "' ";
			echo $sql;
			$result1 = mysqli_query($conn,$sql);
			if(mysqli_num_rows($result1) > 0)
			{
				echo "Sample Type already Exists!";
				exit();
			}
				$sql = "INSERT INTO sampletype_master(sampleType,sampleTypeStatus) VALUES('" . $_POST['txtSamp'] .  "',1)";
				$result = mysqli_query($conn,$sql); 
				echo $sql;
				if($result)
					...
					echo "Unable to save!";
				echo "<script> window.location='SampleType_Master.php';</script>";
					$_POST['btnAdd'] = '';
  ...
  function delService(x) {
	...
	++index;
	var serviceID = document.getElementById("tblSamp").rows[index].cells.item(4).innerHTML;
	...
	if(confirm('Are you sure you want to delete this Service?'))
	{
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
				document.location='SampleType_Master.php';
			}
        };
        xmlhttp.open("GET","del_service.php?service=samp&q="+serviceID,true);
        xmlhttp.send();
	}
